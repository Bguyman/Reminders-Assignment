...
export default class ReminderApp {
  ...
  public start(): void {
    let exitFlag = false;
    for (;;) {  //for (;;) is an infinite for loop
  ...
  private handleToggleCompletion(): void {
    if (this._remindersHandler.reminders.length < 1) {
      Logger.log('\n  ⚠️  You have no reminders')
    } Logger.logReminders(this._remindersHandler.reminders)
    this._remindersHandler.toggleCompletion(Number(this.getUserChoice('reminder number to toggle', true)) - 1)
  ...
  private handleModifyReminders(): void {
    ...
    const index = Number(this.getUserChoice('reminder number to edit', true)) - 1
    const description = this.getUserChoice('new reminder description', false)
    let toggle = ReminderApp.checkUserToggleChoice()
    this._remindersHandler.modifyReminder(index, description)
    Logger.log('\n  🏁   Reminder Modified');
    if (toggle = true) {
  ...
  private handleAddReminder(): void {
    ...
    const tag = this.getUserChoice('reminder tag', false)
  ...
  private handleSearchReminders(): void {
    ...
    const keyword = this.getUserChoice('keyword to search', false)
